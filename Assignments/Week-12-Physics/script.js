...
let tickTimeRight = 5000;
let tickTimeLeft = 5000;
...
function init() {
  ...
  initPhysics();
  createObjects();
  intervalLeft = setInterval(leftSummon, 5000);
  intervalRight = setInterval(rightSummon, 5000);
  initGUI();
  initInput();
}
...
function leftSummon() {
  ...
  if (randomInt == 0) {
    summonSphere(-10,20,-10);
  } else {
    summonBox(-10, 20, -10);
  }
}

function rightSummon() {
  let randomInt = getRandomInt(2);
  if (randomInt == 0) {
    summonSphere(10,20,-10);
  } else {
...
function summonBox(px, py, pz) {
  pos.set(px, py, pz);
  quat.set(0, 0, 0, 1);
  const base1 = createBoxWithPhysics(0.9, 0.9, 0.9, 5.,
                                     pos, quat,
                                     new THREE.MeshPhongMaterial({ color: getRandomColor() }));
}

function summonSphere(px, py, pz) {
  pos.set(px, py, pz);
  quat.set(0, 0, 0, 1);
  const ballMass = 35;
...
function initGUI() {
  ...
  const shaderControlParams = { "Left": tickTimeLeft, "Right": tickTimeRight }
  timeFolder
    .add(shaderControlParams, 'Left', 100, 10000, 50)
    .onChange((value) => updateInterval(1, value));
  timeFolder
    .add(shaderControlParams, 'Right', 100, 10000, 50)
    .onChange((value) => updateInterval(2, value));
  timeFolder.open();
}
 ...
 */
function updateInterval(interval, newTime) {
  if (interval == 1) {
    tickTimeLeft = newTime;
    clearInterval(intervalLeft);
    intervalLeft = setInterval(leftSummon, tickTimeLeft);
  } else if (interval == 2) {
    tickTimeRight = newTime;
    clearInterval(intervalRight);
    intervalRight = setInterval(rightSummon, tickTimeRight);
  }
}
...
function createTargets() {
  pos.set(0,0,-10);
  quat.set(0, 0, 0, 1);
  const base1 = createBoxWithPhysics(0.5, 4.5, 0.5, 100000000000., pos, quat, new THREE.MeshPhongMaterial({ color: getRandomColor() }));
  pos.set(0,5.1,-10);
  quat.set(0, 0, 0, 1);
  const board1 = createBoxWithPhysics(3., 3., 0.5, 100000000000., pos, quat, new THREE.MeshPhongMaterial({ color: 0xffffff }))
  textureLoader.load(
    "https://cdn.glitch.global/8f449212-60fa-483f-925c-df5eca99635f/targetTexture.jpg?v=1669809799355",
    function (texture) {
      board1.material.map = texture;
      board1.material.needsUpdate = true;
    }
  );
  pos.set(10,0,-10);
  quat.set(0, 0, 0, 1);
  const base2 = createBoxWithPhysics(2, 4.5, 2, 100000000000., pos, quat, new THREE.MeshPhongMaterial({ color: getRandomColor() }));
  
  pos.set(-10,0,-10);
  quat.set(0, 0, 0, 1);
  const base3 = createBoxWithPhysics(2, 4.5, 2, 100000000000., pos, quat, new THREE.MeshPhongMaterial({ color: getRandomColor() }));
